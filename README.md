...
GitHub Actions Linux jobs should use something like [`coactions/setup-xvfb`](https://github.com/coactions/setup-xvfb) or `xvfb-run -a` around `pnpm test:e2e`. **`better-sqlite3`** must match the pinned extension-host Electron (the `test:e2e` script runs `rebuild-better-sqlite-for-e2e.mjs` automatically). [`e2e-host-versions.json`](./packages/vscode-ext/scripts/e2e-host-versions.json) has **`electron`** for that VS Code/E2E host and **`electronVsix`** for the Electron ABI used when packaging the public VSIX (often ahead of stock VS Code—e.g. **Cursor**). If you bump the VS Code pin, update **`electron`** (see `microsoft/vscode` `package.json` at that tag). Before tagging a release, align **`electronVsix`** with **Cursor → Help → About** (or DevTools: `process.versions.electron`), or override at build time with **`ELECTRON_EXTENSION_HOST_VERSION`**. For Open VSX publishes, you can set the GitHub repository variable **`ELECTRON_EXTENSION_HOST_VERSION`** so CI rebuilds natives without editing JSON. The launcher **rebuilds `better-sqlite3` back for Node** after E2E so `pnpm test` keeps working; running **`rebuild:e2e-native` alone** still leaves the Electron binary until you run a normal Node rebuild.
...
The extension host runs on **Electron**, not the same Node ABI as the `node` used in CI: after `npm install`, [`package-vsix.mjs`](./packages/vscode-ext/scripts/package-vsix.mjs) runs **`npm rebuild better-sqlite3 --runtime=electron --target=…`** so SQLite loads inside Cursor (avoids `NODE_MODULE_VERSION` and **Module did not self-register** issues). The default target comes from **`electronVsix`** in [`e2e-host-versions.json`](./packages/vscode-ext/scripts/e2e-host-versions.json) (distinct from the **`electron`** field used for E2E against stock VS Code). Override with **`ELECTRON_EXTENSION_HOST_VERSION`** when your installed Cursor build uses a different extension-host Electron.
